...
public class AdViewpagerUtil {
        ...
        if (urls.size() > 0) {
            ...
            for (int i = 0; i < length; i++) {
                if (i < length - 2) {
                    ...
                    ImgLoadUtil.display(context, mImageViews[i + 1], url);
            ...
            ImgLoadUtil.display(context, mImageViews[0], urls.get(urls.size() - 1));
            ImgLoadUtil.display(context, mImageViews[length - 1], urls.get(0));
        }
    }
    ...
    Runnable runnable = new Runnable() {
        ...
        public void run() {
            // 要做的事情
            if (viewPager.getChildCount() > 0 && isLoop) {
