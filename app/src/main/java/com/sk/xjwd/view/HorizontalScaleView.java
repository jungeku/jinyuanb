...
public class HorizontalScaleView extends View {
    ...
    private final int LINE_WIDTH = 3;//指针线宽度
...
//            scaleSpaceUnit = width/5;
            rectWidth = scaleSpaceUnit;
            rectHeight = scaleSpaceUnit / 2;

            borderLeft = width / 2 - ((min + max) / 2 - min)/100 * scaleSpaceUnit;
            borderRight = width / 2 + ((min + max) / 2 - min)/100 * scaleSpaceUnit;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = min; i <= max; i=i+100) {
            ...
            int w = rect.width();
            int h = rect.height();
            float x=minX + (i - min)/100 * scaleSpaceUnit - w / 2 - SCALE_WIDTH_BIG / 2;
            float y=ruleHeight - maxScaleLength - h - minScaleLength / 2;
            canvas.drawText(str, x, y, paint);
            //画大刻度线
            paint.setStrokeWidth(SCALE_WIDTH_BIG);
            paint.setColor(getResources().getColor(R.color.actionsheet_gray));
            float startx=minX + (i - min)/100 * scaleSpaceUnit;
            float starty=ruleHeight - maxScaleLength;
            float stopx=minX + (i - min)/100 * scaleSpaceUnit;
            float stopy=ruleHeight;
...
//        canvas.drawText(value, width / 2 - w2 / 2, ruleHeight + rectPadding + rectHeight / 2 + h2 / 2, paint);
    ...
    private void calculateCurrentScale() {
        ...
        if (originValue - 100*offset > max) {
            currentValue = max;
        } else if (originValue - 100*offset < min) {
            currentValue = min;
        } else {
            currentValue = originValue - 100*offset;
        }
