...
public class PasswordInputView extends EditText {
    ...
    private static final double TEXT_SIZE = 40 * AutoUtils.getTextPixelsRate();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // 密码
        float cx, cy = (float) (height/ 2 + TEXT_SIZE / 2);
        float half = width / passwordLength / 2;
        passwordPaint.setTextSize((float) (TEXT_SIZE));
        for(int i = 0; i < textLength; i++) {
            cx = (float) (width * i / passwordLength + half - TEXT_SIZE / 4);
//            canvas.drawCircle(cx, cy, passwordWidth, passwordPaint);
            canvas.drawText(String.valueOf(getText().toString().charAt(i)),cx,cy,passwordPaint);
        }
    }
