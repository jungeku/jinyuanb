	...
	public void onReceive(Context context, Intent intent) {
		try {
			...
			if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
				...
				String bundleString = bundle.getString(JPushInterface.EXTRA_MESSAGE);
				Logger.d(TAG, "[MyReceiver] 接收到推送下来的自定义消息: " + bundle.getString(JPushInterface.EXTRA_MESSAGE));
				//processCustomMessage(context, bundle);
				//UIUtil.startActivity(MessageActivity.class,null);
				if(bundleString!=null){
					RxBus.getDefault().post(Constants.REQUESTID_0,88);
					manager = (NotificationManager)context.getSystemService(Context.NOTIFICATION_SERVICE);
					Notification.Builder builder1 = new Notification.Builder(context);
					builder1.setSmallIcon(R.mipmap.android_template); //设置图标
					//builder1.setTicker("显示第二个通知");
					builder1.setContentTitle(Api.Appname); //设置标题
					builder1.setContentText("点击查看详细内容"); //消息内容
					builder1.setWhen(System.currentTimeMillis()); //发送时间
					builder1.setDefaults(Notification.DEFAULT_ALL); //设置默认的提示音，振动方式，灯光
					builder1.setAutoCancel(true);//打开程序后图标消失
					Intent intents =new Intent (context,MessageActivity.class);
					intents.putExtra("dian",9);
					PendingIntent pendingIntent =PendingIntent.getActivity(context, 0, intents, 0);
					RxBus.getDefault().post(121,66);
					builder1.setContentIntent(pendingIntent);
					Notification notification1 = builder1.build();
					manager.notify(124, notification1); // 通过通知管理器发送通知
				}
				Log.i(TAG, "onReceive2: "+ bundle.getString(JPushInterface.EXTRA_MESSAGE));
